// SendNotification.jsx - Send notification to a specific batch
...
const SendNotification = ({ open, onClose, onSend }) => {
  ...
  const handleBatchChange = (event, value) => {
    setFormData(prev => ({ ...prev, batch_id: value?.id || null }));
    setFieldErrors(prev => ({ ...prev, batch_id: '' }));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/batches/${formData.batch_id}/send-notification`,
        payload,
        {
          headers: {
    ...
    >
      <DialogTitle sx={{
        ...
        <Typography sx={{ fontSize: '0.7rem', color: COLORS.text.secondary, mt: 0.5 }}>
          Send notifications to students in a specific batch
        </Typography>
      </DialogTitle>
              ...
              <Grid size={{ xs: 12 }}>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                  <Typography sx={{ fontSize: '0.7rem', fontWeight: 600, color: COLORS.text.secondary, letterSpacing: '0.5px' }}>
                    SELECT BATCH <span style={{ color: COLORS.error }}>*</span>
                  </Typography>
                  <Autocomplete
                    options={batches}
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        helperText={fieldErrors.batch_id}
                        sx={textFieldSx}
                      />
                    )}
