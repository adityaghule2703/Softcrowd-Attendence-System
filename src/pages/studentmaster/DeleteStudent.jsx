...
// const DeleteStudent = ({ open, onClose, student, onDelete }) => {
...
const DeleteStudent = ({ open, onClose, student, onDelete }) => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [isDeleted, setIsDeleted] = useState(false); // ✅ Prevent double calls
  ...
  // DeleteStudent.jsx - remove axios import and simplify handleDelete
const handleDelete = async () => {
  if (!student?.id) {
    setError('Invalid student data');
    return;
  ...
  try {
    await onDelete(student.id); // Let parent handle the API call
    onClose();
  } catch (err) {
