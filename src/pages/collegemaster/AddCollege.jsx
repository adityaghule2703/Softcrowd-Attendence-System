...
//       } else {
//         errorMessage = 'Failed to add college. Please try again.';
//       }
...
const AddCollege = ({ open, onClose, onAdd }) => {
  const [formData, setFormData] = useState({
    name: '',
    department_name: [], // Moved up - array for multiple departments
    address: '',
    city: '',
    state: '',
    pincode: '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        name: formData.name.trim(),
        department_name: formData.department_name.map(dept => dept.department_name), // Send array of department names
        address: formData.address.trim(),
        city: formData.city.trim(),
        state: formData.state.trim(),
        pincode: formData.pincode,
        ...
        <Stack spacing={2}>
          ...
          <Paper sx={{ 
            ...
            <Grid container spacing={1.5}>
              ...
              {/* Department Field - Moved right below College Name */}
              <Grid size={{ xs: 12 }}>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                  <Typography sx={{ fontSize: '0.7rem', fontWeight: 600, color: COLORS.text.secondary, letterSpacing: '0.5px' }}>
